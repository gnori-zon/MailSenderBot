...
public class BackCommandTest {
    ...
    private final MessageTypesDistributorService messageTypesDistributorService = Mockito.mock(
        MessageTypesDistributorServiceImpl.class);
    ...
    private void shouldProperlyExecuteCommand(String firstCallBack,String expectedChangedCallBack,State state ) {
        ...
        Mockito.when(modifyDataBaseService.findAccountById(chatId)).thenReturn(
            Optional.of(Account.builder().state(state).build()));
