...
public class ChangeMessageItemCountForRecipientsCallbackCommand implements CallbackCommand {
    ...
    public void execute(Account account, Update update) {
        ...
        accountUpdater.updateState(account.getId(), State.MESSAGE_COUNT_FOR_RECIPIENT_PENDING);

        final long chatId = account.getChatId();
        final int messageId = update.getCallbackQuery().getMessage().getMessageId();
        final String text = textPreparer.prepare(SimpleTextPreparerParam.BEFORE_CHANGE_COUNT_FOR_RECIPIENT);
